    ...
    try: 
        with st.status("Running Engine...", expanded=True) as status:
            ...
            if data.empty:
                ...
                st.write("Extracting and processing live media sentiment layers...")
        ...
        if not data.empty:
            ...
            with tab1:
                ...
                fig.add_trace(go.Scatter(x=t_line, y=[current_price, forecast_w1+u_w1, forecast_w2+u_w2, forecast_w3+u_w3, forecast_w4+u_w4], mode='lines', showlegend=False, line=dict(width=0)))
                fig.add_trace(go.Scatter(x=t_line, y=[current_price, forecast_w1-u_w1, forecast_w2-u_w2, forecast_w3-u_w3, forecast_w4-u_w4], mode='lines', fill='tonexty', fillcolor='rgba(0,230,118,0.03)', name='Tunnel'))
